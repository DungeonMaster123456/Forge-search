...
def get_robot_parser(url: str):
    ...
    Uses requests (with REQUEST_TIMEOUT) instead of RobotFileParser.read(),
    because read() uses urllib under the hood with NO timeout — a slow or
    non-responding server can hang the whole crawler indefinitely.
    """
    parsed = urlparse(url)
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # No robots.txt (404 etc.) => allow everything, per convention
            rp.parse([])
    except requests.RequestException:
        # Unreachable robots.txt: be conservative but don't hang — allow,
        # same as a missing robots.txt, and move on.
        rp.parse([])
